...
# 
#   if "forfar" not in name.lower():
#     continue

  

  # Check if the file already exists
  file_path = f"py/cr/data/{name}_cr_members.csv"
  ...
  url = "https://www.parkrun.org.uk/" + name.replace(" ", "").lower() + "/groups/17402/"
  ...
  if response.status_code == 200:
    ...
    # # Save the entire soup object to an HTML file
    # file_path = f"py/cr/test/{name}_full_page.html"
    ...
    if table:
      ...
      df.to_csv(f"py/cr/data/{name}_cr_members.csv", index=False)
    else:
