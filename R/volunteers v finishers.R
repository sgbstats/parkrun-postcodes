  ...
  if (!is.null(table_index)) {
    ...
    if (length(tables) == 1) {
      ...
      message(
        "Found ",
        length(tables),
        " tables. Returning list of data frames."
      )
      return(tables)
    }
  }
...
preview_html_tables <- function(file_path) {
  ...
  for (i in seq_along(tables)) {
    ...
    if (ncol(tables[[i]]) > 0) {
      cat(
        "  Column names:",
        paste(colnames(tables[[i]]), collapse = ", "),
        "\n"
      )
    }
    cat("  Preview:\n")
...
all_events <- tribble(
  ~"eventname" , ~"event" , ~"date" , ~"finishers" , ~"volunteers"
)
t <- 0
for (i in files) {
  t <- t + 1
    ...
    rbind(
      tables %>%
        mutate(
          ...
          finishers = str_extract(date_first_finishers, "\\d+") %>%
            as.numeric(),
          eventname = new_name
        ) %>%
